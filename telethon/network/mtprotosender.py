        ...
        for attempt in retry_range(self._retries):
            ...
            if not connected:
                raise ConnectionError(
                    f"Connection to Telegram failed {self._retries} time(s)"
                )

            e = ConnectionError(f"auth_key generation failed {self._retries} time(s)")
        ...
        *,
        ...
        try:
            if obj.CONSTRUCTOR_ID in _update_ids:
                ...
                upd = _tl.UpdateShort(
                    _tl.UpdateDeleteMessages([], obj.pts, obj.pts_count), 0
                )
