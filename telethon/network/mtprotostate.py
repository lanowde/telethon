...
class MTProtoState:
    ...
    def write_data_as_message(
        self, buffer, data, content_related, *, after_id=None, msg_id=None
    ):
        """
    ...
    def get_encrypted_bind(self, nonce, auth_key, tmp_auth_key, timestamp):
        ...
        payload = (
            os.urandom(128 // 8) + struct.pack("<qii", msg_id, seq_no, len(bind)) + bind
        )
        # 16-byte alignment
    ...
    def decrypt_message_data(self, body):
        ...
        if obj.CONSTRUCTOR_ID in (
            ...
            if time_delta > MSG_TOO_OLD_DELTA:
                self._log.warning(
                    "Server sent a very old message with ID %d, ignoring (see FAQ for details)",
                    remote_msg_id,
                )
                self._count_ignored()
                return None
            ...
            if -time_delta > MSG_TOO_NEW_DELTA:
                self._log.warning(
                    "Server sent a very new message with ID %d, ignoring (see FAQ for details)",
                    remote_msg_id,
                )
                self._count_ignored()
                return None
