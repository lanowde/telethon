...
class InvalidChecksumError(Exception):
    ...
    def __init__(self, checksum, valid_checksum):
        super().__init__(
            ...
            "This packet should be skipped.".format(checksum, valid_checksum)
        )
...
class CdnFileTamperedError(SecurityError):
    ...
    def __init__(self):
        super().__init__("The CDN file has been altered and its download cancelled.")
...
class BadMessageError(Exception):
    ...
    ErrorMessages = {
        16: "msg_id too low (most likely, client time is wrong it would be "
        ...
        19: "Container msg_id is the same as msg_id of a previously received "
        "message (this must never happen).",
        20: "Message too old, and it cannot be verified whether the server has "
        "received a message with this msg_id or not.",
        32: "msg_seqno too low (the server has already received a message with a "
        "lower msg_id but with either a higher or an equal and odd seqno).",
        33: "msg_seqno too high (similarly, there is a message with a higher "
        "msg_id but with either a lower or an equal and odd seqno).",
        34: "An even msg_seqno expected (irrelevant message), but odd received.",
        35: "Odd msg_seqno expected (relevant message), but even received.",
        48: "Incorrect server salt (in this case, the bad_server_salt response "
    ...
    def __init__(self, request, code):
        super().__init__(
            ...
            self.ErrorMessages.get(
                code, "Unknown error code (this should not happen): {}.".format(code)
            ),
        )
