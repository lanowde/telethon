...
def _raise_cast_fail(entity, target):
    raise TypeError(f"Cannot cast {type(entity).__name__} to any kind of {target}.")
...
def get_input_media(
    ...
    *,
    ...
    if isinstance(media, (types.Photo, types.photos.Photo, types.PhotoEmpty)):
        return types.InputMediaPhoto(
            id=get_input_photo(media), ttl_seconds=ttl, spoiler=spoiler
        )
    ...
    if isinstance(media, (types.Document, types.DocumentEmpty)):
        return types.InputMediaDocument(
            id=get_input_document(media), ttl_seconds=ttl, spoiler=spoiler
        )

    if isinstance(media, (types.InputFile, types.InputFileBig)):
        if is_photo:
            return types.InputMediaUploadedPhoto(
                file=media, ttl_seconds=ttl, spoiler=spoiler
            )
...
def is_image(file):
    ...
    """
    if match := re.match(r"\.(png|jpe?g)", _get_extension(file), re.IGNORECASE):
        return True
    else:
...
def get_appropriated_part_size(file_size):
    ...
    if file_size <= 104857600:  # 100MB
        return 128
    return 256 if file_size <= 786432000 else 512
