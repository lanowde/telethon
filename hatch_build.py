...
class CustomBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict[str, Any]) -> None:
        ...
        layer = next(
            filter(None, map(lambda p: find_layer(self.root / p), TLOBJECT_IN_TLS))
        )
        errors = list(parse_errors(self.root / ERRORS_IN))
        methods = list(
            parse_methods(
                self.root / METHODS_IN,
                self.root / FRIENDLY_IN,
        ...
        tlobjects = list(
            itertools.chain(
                *(
                    parse_tl(self.root / file, layer, methods)
        ...
        self.clean([])
        generate_tlobjects(
            tlobjects, layer, IMPORT_DEPTH, self.directory / TLOBJECT_OUT
        )
        ...
        build_data["force_include"][str(self.directory / LIBRARY_DIR)] = str(
            LIBRARY_DIR
        )
