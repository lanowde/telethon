...
async def test_send_message_with_file_forwards_args():
    ...
    for value, name in enumerate(
        inspect.signature(TelegramClient.send_message).parameters
    ):
...
class TestMessageMethods:
    ...
    @pytest.mark.parametrize(
        "formatting_entities", ([MessageEntityBold(offset=0, length=0)], None)
    )
    async def test_send_msg_and_file(self, formatting_entities):
        ...
        with mock.patch.object(
            target=MessageMethods,
            attribute="send_file",
            new=MagicMock(return_value=async_func(expected_result)),
            create=True,
        ) as mock_obj:
