...
async def payment_pre_checkout_handler(event: types.UpdateBotPrecheckoutQuery):
    if event.payload.decode("UTF-8") == "product A":
        ...
        await bot(
            functions.messages.SetBotPrecheckoutResultsRequest(
                query_id=event.query_id, success=False, error="Something went wrong"
            )
        )
    ...
    if isinstance(event.message.action, types.MessageActionPaymentSentMe):
        ...
        if payment.payload.decode("UTF-8") == "product A":
            await bot.send_message(
                event.message.peer_id.user_id, "Thank you for buying product A!"
            )
        elif payment.payload.decode("UTF-8") == "product B":
            await bot.send_message(
                event.message.peer_id.user_id, "Thank you for buying product B!"
            )
        raise events.StopPropagation
...
def generate_invoice(
    ...
    price = types.LabeledPrice(
        label=price_label, amount=price_amount
...
async def start_handler(event: events.NewMessage.Event):
    await bot.send_message(
        ...
        file=generate_invoice(
            ...
            currency="RUB",
            title="Title C",
            description="description c - shall cause an error",
            payload="product C",
            start_param="abc",
        ),
    )
