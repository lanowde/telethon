...
class InteractiveTelegramClient(TelegramClient):
            ...
            while True:
                ...
                if msg == "!q":
                    ...
                    for msg in reversed(messages):
                        ...
                        sprint(
                            f"[{msg.date.hour}:{msg.date.minute}] (ID={msg.id}) {name}: {content}"
                        )
                    ...
                    output = await self.download_profile_photo(entity, "usermedia")
                    if output:
    ...
    async def send_photo(self, path, entity):
        ...
        await self.send_file(
            entity, path, progress_callback=self.upload_progress_callback
        )
        print("Photo sent!")
