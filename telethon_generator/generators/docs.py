...
def _generate_index(folder, paths, bots_index=False, bots_index_paths=()):
    ...
    with DocsWriter(filename, _get_path_for_type) as docs:
        ...
        if bots_index:
            docs.write_text(
                "These are the methods that you may be able to "
                'use as a bot. Click <a href="{}">here</a> to '
                "view them all.".format(INDEX)
            )
        else:
            docs.write_text(
                'Click <a href="{}">here</a> to view the methods '
    ...
    for tlobject in tlobjects:
        ...
        with DocsWriter(filename, _get_path_for_type) as docs:
            ...
            if tlobject.is_function:
                ...
                docs.write_text(
                    '{} use this method. <a href="#examples">'
            ...
            if tlobject.result == generic_arg:
                ...
                docs.write_text(
                    "This function returns the result of whatever "
    ...
    for t, cs in type_to_constructors.items():
        ...
        with DocsWriter(filename, _get_path_for_type) as docs:
            ...
            if not other_methods:
                docs.write_text("No methods accept this type as an input parameter.")
            elif len(other_methods) == 1:
                docs.write_text("Only this method has a parameter with this type.")
            else:
                docs.write_text(
