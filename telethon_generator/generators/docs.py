...
def _generate_index(folder, paths, bots_index=False, bots_index_paths=()):
    ...
    with DocsWriter(filename, _get_path_for_type) as docs:
        ...
        docs.write_head(
            str(folder).replace(os.path.sep, "/").title(),
            css_path=paths["css"],
        ...
        _build_menu(docs)
        docs.write_title(str(filename.parent).replace(os.path.sep, "/").title())

        if bots_index:
            docs.write_text(
                f'These are the requests that you may be able to use as a bot. Click <a href="{INDEX}">here</a> to view them all.'
            )
        else:
            docs.write_text(
                f'Click <a href="{BOT_INDEX}">here</a> to view the requests that you can use as a bot.'
            )
        if namespaces:
            ...
            for namespace in namespaces:
                ...
                if bots_index:
                    namespace_paths.extend(
                        item for item in bots_index_paths if item.parent == namespace
                    )
                _generate_index(namespace, paths, bots_index, namespace_paths)
    ...
    for tlobject in tlobjects:
        ...
        with DocsWriter(filename, _get_path_for_type) as docs:
            ...
            if tlobject.is_function:
                ...
                docs.write_text(
                    '{} use this request. <a href="#examples">'
                    "See code examples.</a>".format(start)
                )

            # Write the code definition for this TLObject
            docs.write_code(tlobject)
            docs.write_copy_button(
                "Copy import to the clipboard", get_import_code(tlobject)
            )
            ...
            generic_arg = next(
                (arg.name for arg in tlobject.args if arg.generic_definition), None
            )
            ...
            if tlobject.result == generic_arg:
                ...
                docs.write_text(
                    "This request returns the result of whatever "
            ...
            if tlobject.is_function:
                ...
                if tlobject.friendly:
                    ...
                    docs.write_text(
                        ...
                        "examples on how to use it.".format(ns, friendly)
                    )
                    docs.write_text(
...
<strong>with</strong> TelegramClient(name, api_id, api_hash) <strong>as</strong> client:
    ...
    <strong>for</strong> x <strong>in</strong> result:
        print(x"""
                    ...
                    if tlobject.result != "Bool" and not tlobject.result.startswith(
                        "Vector"
                    ):
    ...
    for t, cs in type_to_constructors.items():
        ...
        with DocsWriter(filename, _get_path_for_type) as docs:
            ...
            if not cs:
                ...
                docs.write_text("This type has %d constructors available." % len(cs))
            ...
            if not other_methods:
                docs.write_text("No request accepts this type as an input parameter.")
            elif len(other_methods) == 1:
                docs.write_text("Only this request has a parameter with this type.")
            else:
                docs.write_text(
    ...
    _generate_index(pathlib.Path("methods"), paths, True, bot_docs_paths)
    ...
    def fmt(xs, formatter):
        return ", ".join(
            '"{}"'.format(formatter(x)).replace(os.path.sep, "/") for x in xs
        )
