...
def get_import_code(tlobject):
    ...
    return "from telethon.tl.{}{} import {}".format(kind, ns, tlobject.class_name)
...
def _generate_index(folder, paths, bots_index=False, bots_index_paths=()):
    ...
    with DocsWriter(filename, _get_path_for_type) as docs:
        ...
        if bots_index:
            docs.write_text(
                "These are the requests that you may be able to "
                'use as a bot. Click <a href="{}">here</a> to '
                "view them all.".format(INDEX)
            )
        else:
            docs.write_text(
                'Click <a href="{}">here</a> to view the requests '
