...
NAMED_AUTO_CASTS = {("chat_id", "int"): "await client.get_peer_id({}, add_mark=False)"}
...
NAMED_BLACKLIST = {"messages.discardEncryption"}
...
def _write_modules(out_dir, depth, kind, namespace_tlobjects, type_constructors):
    ...
    for ns, tlobjects in namespace_tlobjects.items():
        ...
        with file.open("w") as f, SourceBuilder(f) as builder:
            ...
            if not ns:
                builder.writeln(
                    "from . import {}",
                    ", ".join(sorted(x for x in namespace_tlobjects.keys() if x)),
                )
            ...
            for t in tlobjects:
                if not t.is_function:
                    ...
                    if not constructors:
                        ...
                        type_defs.append(
                            f"Type{type_name} = {constructors[0].class_name}"
                        )
                    else:
                        type_defs.append(
                            f"Type{type_name} = Union[{','.join(c.class_name for c in constructors)}]"
                        )
            ...
            for t in tlobjects:
                for arg in t.args:
                    ...
                    import_space = f"{'.' * depth}.tl.types"
                    if "." in name:
                        namespace = name.split(".")[0]
                        name = name.split(".")[1]
                        import_space += f".{namespace}"
                    ...
                    if name not in type_names:
                        ...
                        imports[import_space].add(f"Type{name}")
            ...
            if imports:
                ...
                for namespace, names in imports.items():
                    builder.writeln(
                        "from {} import {}", namespace, ", ".join(sorted(names))
                    )
...
def _write_class_init(tlobject, kind, type_constructors, builder):
    ...
    builder.writeln("SUBCLASS_OF_ID = {:#x}", crc32(tlobject.result.encode("ascii")))
    builder.writeln()

    # Convert the args to string parameters, those with flag having =None
    args = [
        f"{a.name}: {a.type_hint()}{'=None' if a.flag or a.can_be_inferred else ''}"
    ...
    if not constructors:
        ...
        builder.writeln("Instance of {}.", constructors[0].class_name)
    else:
        builder.writeln(
            "Instance of either {}.", ", ".join(c.class_name for c in constructors)
        )
    ...
    for arg in tlobject.real_args:
        if not arg.can_be_inferred:
            ...
            code = f"int.from_bytes(os.urandom({8 if arg.type == 'long' else 4}), 'big', signed=True)"
            ...
            if arg.is_vector:
                ...
                code = f"[{code} for _ in range(len(id))]"
...
def _write_to_dict(tlobject, builder):
    ...
    for arg in tlobject.real_args:
        ...
        if arg.type in BASE_TYPES:
            ...
            builder.write(
                "self.{0}.to_dict() " "if isinstance(self.{0}, TLObject) else self.{0}",
                arg.name,
            )
...
def _write_to_bytes(tlobject, builder):
    ...
    for ra in repeated_args.values():
        if len(ra) > 1:
            cnd1 = ("(self.{0} or self.{0} is not None)".format(a.name) for a in ra)
            cnd2 = (
                "(self.{0} is None or self.{0} is False)".format(a.name) for a in ra
            )
            builder.writeln(
...
def _write_from_reader(tlobject, builder):
    ...
    builder.writeln(
        "return cls({})",
        ", ".join("{0}=_{0}".format(a.name) for a in tlobject.real_args),
    )
...
def _write_arg_read_code(builder, arg, tlobject, name):
    ...
    if arg.flag:
        ...
        if arg.type == "true":
            builder.writeln("{} = bool({} & {})", name, arg.flag, 1 << arg.flag_index)
            return
    ...
    if arg.is_vector:
        ...
        ns, t = (
            (".", arg.type)
            if sep_index == -1
            else (f".{arg.type[:sep_index]}", arg.type[sep_index + 1 :])
        )
