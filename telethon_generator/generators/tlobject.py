            ...
            for t in tlobjects:
                if not t.is_function:
                    ...
                    if not constructors:
                        ...
                        type_defs.append(
                            "Type{} = {}".format(type_name, constructors[0].class_name)
                        )
                    else:
                        type_defs.append(
                            "Type{} = Union[{}]".format(
                                type_name, ",".join(c.class_name for c in constructors)
                            )
                        )
...
def _write_class_init(tlobject, kind, type_constructors, builder):
    ...
    for arg in tlobject.real_args:
        ...
        # inferred are those called 'random_id'
            ...
            code = "int.from_bytes(os.urandom({}), 'big', signed=True)".format(
                8 if arg.type == "long" else 4
            )
    ...
    if tlobject.is_function and any(
        ...
        for arg in tlobject.real_args:
            ...
            if arg.is_vector:
                ...
                builder.writeln("self.{} = {}", arg.name, ac.format("self." + arg.name))
