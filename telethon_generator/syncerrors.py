...
def main():
    ...
    def get_desc(code):
        return re.sub(
            r"\s*&\w+;\s*", "", (tg_data["descriptions"].get(code) or "").rstrip(".")
        )
    ...
    new_errors.extend(
        (e.str_code, " ".join(map(str, sorted(set(e.int_codes)))), e.description)
        for e in self_errors.values()
    )
    new_methods.extend(
        (m.name, m.usability.key, " ".join(sorted(e.str_code for e in m.errors)))
        for m in self_methods.values()
    )

    csv.register_dialect("plain", lineterminator="\n")
    with ERRORS.open("w", encoding="utf-8", newline="") as fd:
        csv.writer(fd, "plain").writerows(
            (("name", "codes", "description"), *sorted(new_errors))
        )
    with METHODS.open("w", encoding="utf-8", newline="") as fd:
        csv.writer(fd, "plain").writerows(
            (("method", "usability", "errors"), *sorted(new_methods))
        )
