...
def _get_class_name(error_code):
    ...
    if re.match(r"\d+", error_code):
        raise RuntimeError(
            "error code starting with a digit cannot have valid Python name: {}".format(
                error_code
            )
        )
    ...
    with csv_file.open(newline="") as f:
        ...
        for line, tup in enumerate(f, start=2):
            try:
                ...
                raise ValueError(
                    "Columns count mismatch, unquoted comma in "
                    "desc? (line {})".format(line)
                ) from None
            ...
            try:
                ...
                raise ValueError(
                    "Not all codes are integers " "(line {})".format(line)
                ) from None
