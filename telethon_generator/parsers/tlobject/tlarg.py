    ...
    for d in dicts:
        for k, v in d.items():
            if v in ("None", "True", "False"):
                ...
                d[k] = re.sub(
                    r'([brf]?([\'"]).*\2)', lambda m: f"<em>{m.group(1)}</em>", v
                )
...
class TLArg:
    def __init__(self, name, arg_type, generic_definition):
        ...
        if arg_type == "#":
            ...
            if self.type == "int" and (
                re.search(r"(\b|_)(date|until|since)(\b|_)", name)
                or name in ("expires", "expires_at", "was_online")
            ):
                self.type = "date"
    ...
    def omit_example(self):
        return (self.flag or self.can_be_inferred) and self.name in OMITTED_EXAMPLES
