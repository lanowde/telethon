...
def _from_line(line, is_function, method_info, layer):
    ...
    args_match = re.findall(r"({)?" r"(\w+)" r":" r"([\w\d<>#.?!]+)" r"}?", line)
    ...
    return TLObject(
        ...
        friendly=friendly,
        args=[
            TLArg(name, arg_type, brace != "") for brace, name, arg_type in args_match
        ],
    )
...
def parse_tl(file_path, layer, methods=None, ignored_ids=CORE_TYPES):
    ...
    with file_path.open() as file:
        ...
        for line in file:
            ...
            try:
                result = _from_line(line, is_function, method_info, layer=layer)
